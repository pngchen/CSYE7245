...
from synrad_generator import get_synrad_generator
import gcsfs
import datetime
...
def main():
    ...
    trn_generator = get_synrad_generator(sevir_catalog="gs://assignment1-data/CATALOG.csv",
                                         sevir_location="gs://assignment1-data/data",
                                         start_date=datetime.datetime(2018,6,1),
                                         end_date=datetime.datetime(2018,6,2))
    logger.info('Reading/writing training data to %s' % ('%s/synrad_training.h5' % '../../data/interim'))
    read_write_chunks('%s/synrad_training.h5' % '../../data/interim',trn_generator,20)
