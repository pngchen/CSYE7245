...
def visualize_result(modelName, x_test, y_test, idx, ax):
    ...
    # Image.fromarray(y_pred[0, :, :, 0]).convert('RGB').save("res.jpg", format='jpeg')
...
if application == "Nowcast":
    ...
    if st.button("Nowcast"):
        if path != '' and model is not None:
            ...
            x_test, y_test = dataPipeline.run(path, 1)
            fig, ax = plt.subplots(4, 7, figsize=(20, 8), gridspec_kw={'width_ratios': [1, .2, 1, .2, 1, 1, 1]})
            visualize_result(models[model], x_test, y_test, idx, ax)
